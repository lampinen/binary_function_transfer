...
t1s = c("XOR_of_XORs", "XOR", "XAO", "AND")
t2s = c("X0", "XOR", "XOR_of_XORs", "XAO", "OR", "AND", "None")
lrs = c(0.005, 0.001)
nhs = c(10)
ims = c(1.)
num_runs = 50
directory_format = "results_synaptic_intelligence_nh_%i_lr_%.4f_im_%.2f/"
...
ggsave("plots/median_results_summary_synaptic_intelligence.png")
```
...
for (this_t1 in c("XOR", "XOR_of_XORs", "XAO", "AND")) {
  ...
  ggsave(sprintf("plots/%s_stopping_by_t2_distributions_synaptic_intelligence.png", this_t1), width=7, height=10)
}
```
...
# synaptic intelligence weight analyses
        ...
        for (init_mult in ims) {
          ...
          for (t1 in t1s) {
            for (t2 in t2s) {
              ...
              filename = sprintf("%st1%s_t2%s_sharedinput%s_run%i_reg_strengths.csv", directory, t1, t2, input_shared, run)
              if (!file.exists(filename)) {
                next
              }
              this_d = tryCatch(read.csv(filename, header=T),
...
```{r}
ggplot(data=si_d %>%
         mutate(weight_array = factor(weight_array, levels=c("Bidom2:0", "Widom2:0", "B1:0", "Wh1:0", "B2:0", "Wh2:0", "Bodom2:0", "Wodom2:0"))),
       aes(color=t2, x=value)) +
  geom_density() +
  facet_wrap(. ~ weight_array,
             scales="free_y") +
  scale_x_log10() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="Synaptic intelligence weight (log)")
ggsave("plots/synaptic_intelligence_reg_strengths.png", width=8, height=4)
```

```{r}
si_d %>%
  group_by(weight_array,  run, t1, t2) %>%
  summarize(max_val = max(value)) %>%
  ungroup() %>%
  group_by(weight_array, t2) %>%
  summarize(mean_max_val = mean(max_val),
            sd_max_val = sd(max_val))
```
