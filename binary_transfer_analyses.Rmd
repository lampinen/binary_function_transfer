...
t1s = c("XOR_of_XORs", "XAO", "XOR", "AND", "ANDXORS")
t2s = c("X0", "XOR", "XOR_of_XORs", "XAO", "OR", "AND", "ANDXORS", "None")
lrs = c(0.005)
nhs = c(10)
ims = c(0.33)
num_runs = 100
directory_format = "results_generalization_2_nh_%i_lr_%.4f_im_%.2f/"
...
min_generalization_d_2 = d %>%
  ...
  ungroup() %>%
  filter(task_domain == "loss1") %>%
  group_by(lr, num_hidden, init_size, t1, t2, input_shared, run, successful_learning_source) %>%
...
stopping_d_2 = d %>%
  ...
  group_by(lr, num_hidden, init_size, t1, t2, input_shared, successful_learning_source, run) %>% 
...
```{r}
ggplot(min_generalization_d_2, aes(x=t1, y=min_gen_target, fill=t2)) +
  geom_bar(stat="summary",
           fun.y="mean",
           position="dodge") +
  geom_errorbar(stat="summary",
...
```{r}
ggplot(stopping_d_2, aes(x=t1, y=epoch_005-20000, color=t2)) +
  geom_point(stat="summary",
           fun.y="mean",
           position=position_dodge(0.9)) +
  geom_errorbar(stat="summary",
