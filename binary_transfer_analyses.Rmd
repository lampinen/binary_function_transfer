...
t1s = c("XOR_of_XORs", "XOR", "AND")
t2s = c("X0", "XOR", "XOR_of_XORs", "OR", "AND", "None")
lrs = c(0.005, 0.001)
nhs = c(40, 10)
ims = c(1., 0.33)
num_runs = 10
directory_format = "results_nh_%i_lr_%.4f_im_%.2f/"
...
```{r}
ggplot(partly_more_summarized_d %>% filter(lr == 0.005, num_hidden==10, init_size==0.33, t1=="XOR"),
       aes(x=epoch_001, color=t2)) +
  geom_density() +
  geom_vline(data=more_summarized_d %>% ungroup() %>% filter(lr == 0.005, num_hidden==10, init_size==0.33, t1=="XOR"),
...
```{r}
ggsave("plots/XOR_stopping_by_t2_distributions.png", width=7, height=10)
```
