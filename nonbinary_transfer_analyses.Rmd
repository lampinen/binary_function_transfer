...
gtbs = c(5, 10)
num_runs = 500
directory_format = "results_generalization_nonbinary_stb_gtb_%i_nl_%i_nh_%i_lr_%.4f_im_%.2f/"
        ...
        for (num_hidden in nhs) {
          for (lr in lrs) {
            for (init_mult in ims) {
              directory = sprintf(directory_format, ground_truth_bottleneck, num_layers, num_hidden, lr, init_mult)
              for (t1 in t1s) {
                for (t2 in t2s) {
...
```{r}
success_pct = 0.25  # what percent of the original test error you must reach to be considered a success
...
d_2 = d %>%
  ...
  group_by(lr, ground_truth_bottleneck, num_layers, num_hidden, init_size, t1, t2, input_shared, run, task_domain, train_or_test) %>%
...
min_generalization_d_2 = d_2 %>%
  group_by(lr, ground_truth_bottleneck, num_layers, num_hidden, init_size, t1, t2, input_shared, run) %>%
  ...
  mutate(prior_task = case_when(t2 == "None" ~ "None",
                                t2 == t1 ~ "Isomorphic",
                                T ~ "Non-isomorphic")) %>%
  group_by(lr, ground_truth_bottleneck, num_layers, num_hidden, init_size, t1, t2, prior_task, input_shared, run, successful_learning_source) %>%
  summarize(min_gen_target = min(test)) %>%
  ungroup() 

min_generalization_correlation_d_2 = d_2 %>%
  mutate(prior_task = case_when(t2 == "None" ~ "None",
                                t2 == t1 ~ "Isomorphic",
                                T ~ "Non-isomorphic")) %>%
  group_by(lr, ground_truth_bottleneck, num_layers, num_hidden, init_size, t1, t2, prior_task, input_shared, run, task_domain) %>%
...
successful_generalization_d_2 = d_2 %>%
  group_by(lr, ground_truth_bottleneck, num_layers, num_hidden, init_size, t1, t2, input_shared, run, task_domain) %>%
  top_n(1, wt=epoch) %>% 
  ungroup() %>%
  mutate(prior_task = case_when(t2 == "None" ~ "None",
                                t2 == t1 ~ "Isomorphic",
                                T ~ "Non-isomorphic")) %>%
  group_by(lr, ground_truth_bottleneck, num_layers, num_hidden, init_size, t1, t2, prior_task, input_shared, run) %>%
  summarize(successful_learning_source = any(task_domain=="loss2" & test < success_pct),
            successful_learning_target = any(task_domain=="loss1" & epoch >= 40000 & test < success_pct)) %>%
  ungroup() 
...
stopping_d_2 = d_2 %>%
  ...
  ungroup() %>%
  filter(task_domain == "loss1" & epoch > 40000) %>%
  mutate(prior_task = case_when(t2 == "None" ~ "None",
                                t2 == t1 ~ "Isomorphic",
                                T ~ "Non-isomorphic")) %>%
  group_by(lr, ground_truth_bottleneck, num_layers, num_hidden, init_size, t1, t2, prior_task, input_shared, successful_learning_source, run) %>% 
...
ggplot(successful_generalization_d_2, aes(x=prior_task, y=1*successful_learning_target, fill=prior_task)) +
  ...
  facet_grid(ground_truth_bottleneck + num_hidden ~successful_learning_source)
```


```{r}
ggplot(min_generalization_d_2, aes(x=prior_task, y=min_gen_target, fill=prior_task)) +
  geom_bar(stat="summary",
           fun.y="mean",
           position="dodge") +
  geom_errorbar(stat="summary",
  ...
  facet_wrap(num_hidden ~ ground_truth_bottleneck)
#ggsave("plots/transfer_nonbinary_tasks.png", width=6, height=4)
```

```{r}
ggplot(min_generalization_correlation_d_2, aes(x=min_gen_source, y=min_gen_target, color=prior_task)) +
  geom_point(alpha=0.1) +
  geom_point(data=min_generalization_correlation_d_2 %>% 
               filter(prior_task=="None"),
             aes=aes(x=0),
             stat="summary",
             fun.y="mean",
             size=5) +
  ...
  scale_fill_brewer(palette="Dark2") +
  labs(x="Prior task optimal test error", y="Target task optimal test error") +
  guides(fill=guide_legend(title="Prior task")) +
  ylim(0, NA) +
  facet_wrap(num_hidden~ground_truth_bottleneck)
#ggsave("plots/nonbinary_min_gen_correlation.png", width=6, height=4)
```

```{r}
ggplot(stopping_d_2, aes(x=prior_task, y=epoch_success-40000, color=prior_task)) +
  ...
  facet_grid(lr ~ successful_learning_source)
```