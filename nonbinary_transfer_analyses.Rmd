...
optimizer = "Adam"
num_runs = 200
directory_format = "results_generalization_nonbinary_%s_stb_gtb_%i_nl_%i_nh_%i_lr_%.4f_im_%.2f/"
        ...
        for (num_hidden in nhs) {
          for (lr in lrs) {
            for (init_mult in ims) {
              directory = sprintf(directory_format, optimizer, ground_truth_bottleneck, num_layers, num_hidden, lr, init_mult)
              for (t1 in t1s) {
                for (t2 in t2s) {
...
ggplot(min_generalization_difference_d_2, aes(x=iso_vs_noniso)) +
  ...
  geom_vline(data=min_generalization_difference_d_2 %>%
               group_by(lr, ground_truth_bottleneck, num_layers, num_hidden, init_size, t1, input_shared) %>%
               summarize(iso_vs_noniso_mean = mean(iso_vs_noniso)) %>%
               ungroup(),
...
ggplot(stopping_d_2, aes(x=prior_task, y=epoch_success-10000, color=prior_task)) +
  ...
  facet_grid(ground_truth_bottleneck + num_hidden ~ successful_learning_source, scales="free")
```